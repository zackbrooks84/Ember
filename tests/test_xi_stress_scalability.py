...
# Mark so it can be filtered, but still runs by default.
...
def _time_compute(sample: str, repeats: int = 3) -> Tuple[float, float]:
    """
    Returns (best_time, xi_value). We take the best-of-N to reduce noise.
    """
    best = float("inf")
    xi_val = None
    for _ in range(repeats):
        t0 = time.perf_counter()
        xi_val = compute_xi(sample)
...
def _scaling_guard(baseline: float, large: float) -> None:
    ...
    """
    # If baseline is extremely tiny, clamp to a sensible floor to avoid division noise
    baseline = max(baseline, 1e-4)
    ratio = large / baseline

    # Default guard: 80x slower for huge input is still acceptable in CI.
    # You can tighten with env var, e.g. PERF_MAX_RATIO=40
...
def test_xi_scalability_baseline_vs_huge():
    ...
    """
    # Small baseline (~a few hundred chars)
    small = _make_text(n_sentences=5, with_anchors=True)
    # Very large (~hundreds of thousands of chars)
